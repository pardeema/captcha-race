...
export function loadScoresLocal(): ScoreRow[] {
  try {
...
export function saveScoresLocal(rows: ScoreRow[]): void {
  try {
...
// Remote API helpers
export async function loadScoresRemote(): Promise<ScoreRow[]> {
  try {
...
export async function saveScoreRemote(score: ScoreRow): Promise<ScoreRow[]> {
  try {
    const response = await fetch(API_BASE, {
      method: 'POST',
      headers: {
...
export async function saveScore(score: ScoreRow): Promise<ScoreRow[]> {
  // Try remote first
